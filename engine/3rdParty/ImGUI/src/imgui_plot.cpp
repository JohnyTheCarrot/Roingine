	...
	PlotStatus Plot(const char *label, const PlotConfig &conf) {
		...
		if (conf.values.count > 0) {
			...
			const float inv_scale =
			        (conf.scale.min == conf.scale.max) ? 0.0f : (1.0f / (conf.scale.max - conf.scale.min));
					...
					case PlotConfig::Scale::Log10: {
						...
						while (start < x_max) {
							for (int i = 1; i < 10; ++i) {
								...
								window->DrawList->AddLine(
								        ImVec2(x0, y0), ImVec2(x0, y1), IM_COL32(200, 200, 200, (i > 1) ? 128 : 255)
								);
							}
							start *= 10.f;
						}
			...
			if (conf.grid_y.show) {
				...
				for (int i = 0; i <= cnt; ++i) {
					...
					window->DrawList->AddLine(
					        ImVec2(x0, y0), ImVec2(x1, y0), IM_COL32(0, 0, 0, (i % conf.grid_y.subticks) ? 16 : 64)
					);
				}
			}
		...
		if (conf.overlay_text)
			RenderTextClipped(
			        ImVec2(frame_bb.Min.x, frame_bb.Min.y + style.FramePadding.y), frame_bb.Max, conf.overlay_text,
